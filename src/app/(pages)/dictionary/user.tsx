...
export default function UserDictionary() {
  ...
  const [state, dispatch] = useReducer(
    ...
    InitialDictionary,
    () => ({
      words: JSON.parse(localStorage.getItem("wordDoc") ?? "[]"),
      selectedTags: [],
      selectedTypes: [],
      dup: false,
      search: "",
  ...
  useEffect(() => {
    if (!Name.current?.value && !state.selectedTypes) return;

    const handler = setTimeout(() => {
      localStorage.setItem("wordDoc", JSON.stringify(state.words));
    }, 500);
                        ...
                        <div className="flex items-center justify-between space-x-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <motion.button
                              ...
                              aria-label="favorite"
                              onClick={() =>
                                UserFavorite(word, dispatch, toastPopUp)
                              }
                            >
                              <FontAwesomeIcon
                                icon={faStar}
